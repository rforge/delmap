...
a <- ReadData("test.dat", line.names=TRUE, na.strings="yyy")
...
CreateDistMetric <- function(mat=NULL)
{
...
## Return:   distance object for deletion data
   ...
   S <- S - diag(diag(S))
    ...
    dist.disS <- as.dist(disS)

    return(dist.disS)

} ## end function CreateDistMetric
...
library(TSP,       lib.loc="~/RLib")
library(gclus,     lib.loc="~/RLib")
library(seriation, lib.loc="~/RLib")
library(fields)



## simulate data
res <- SimDeletions(numlines=20, plines=0.6, nummarkers=40, Enumdel=5, p.missing=0.05, seed=1)
S <- res$nomissing


## CleanData (lines and markers with no deletions are removed
cleanS <- CleanData(S)


### Visual inspection of simulated data
#
#seriate(cleanS, method="BEA")
#a <- seriate(cleanS, method="BEA")
#get_order(a[1])
#
#
#ypos <- seq(1, nrow(cleanS), 1)
#xpos <- seq(1, ncol(cleanS), 1)
#image(x=xpos, y=ypos, z=t(cleanS[get_order(a[1]),] ), xlab="markers", ylab="lines",  xaxt="n")
#axis(1, at=1:ncol(cleanS), labels= colnames(cleanS), las=2)
#
#grid(nx=ncol(cleanS), ny=nrow(cleanS), col="blue")
#
...
#       axis(1, at=1:ncol(x), labels= colnames(x), las=2 )
#       axis(2, at=1:nrow(x), labels=NULL)
...
permS <- PermuteCols(cleanS)

## form distance metric for simulated data 
D  <- CreateDistMetric(permS)
###----------------------##
##   TSP                 ##
##-----------------------##
library(TSP, lib.loc="~/RLib")
...
## convert into TSP object
tsp <- as.TSP(D)

## add an extra "city" to find shortest Hamiltonian path for TSP
tsp <- insert_dummy(tsp, label="cut")

a <- solve_TSP(tsp, method="concorde")
## report results

b <-  cut_tour(a, "cut")
#b <- embed(names(b),2)[,2:1] 
#ts[[rep]] <- paste(b[,1], b[,2], sep=".")
ts[[rep]] <- paste( names(cut_tour(a, "cut")) , collapse=".")
}
